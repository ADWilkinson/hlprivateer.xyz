...
- Hyperliquid trading key:
  - Stored as encrypted file under `/etc/hlprivateer/keys/hyperliquid.key.age`.
  - Decrypted at boot with `systemd-ask-password` + age identity.
  - Never logged, never sent over websocket/API.
- JWT/x402 signing keys:
  - Ed25519 key pairs rotated every 30 days.
  - Public keys exposed via JWKS endpoint for verifier integration.
- Secret handling:
  - `SOPS + age` for repo-managed encrypted config blobs.
  - Runtime secrets mounted through systemd credentials.
...
- Secrets: `SOPS + age + systemd credentials`.
