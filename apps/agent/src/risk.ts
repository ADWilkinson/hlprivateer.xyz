...
export interface RiskContext {
  ...
  config: RiskConfig
  recentSignals: readonly SentimentItem[]
...
function gateInvalidProposal(ctx: RiskContext): RiskGateFailure | null {
  ...
  if (p.marketId !== ctx.market.id) {
    return { code: 'INVALID_PROPOSAL', reason: 'proposal/market id mismatch' }
  }
  return null
}
...
function gateStaleSentiment(ctx: RiskContext): RiskGateFailure | null {
  const now = ctx.nowMs ?? Date.now()
  const consideredMs = Date.parse(ctx.proposal.signalsConsideredAt)
  if (Number.isNaN(consideredMs)) {
    return {
      code: 'STALE_SENTIMENT',
      reason: 'proposal.signalsConsideredAt is unparseable',
      observed: ctx.proposal.signalsConsideredAt
    }
  }
  const ageSec = Math.floor((now - consideredMs) / 1000)
  if (ageSec > ctx.config.maxSentimentAgeSec) {
    return {
      code: 'STALE_SENTIMENT',
      reason: `sentiment considered ${ageSec}s ago > maxAge ${ctx.config.maxSentimentAgeSec}s`,
      observed: ageSec,
  ...
  if (ctx.recentSignals.length === 0) {
    return {
      code: 'STALE_SENTIMENT',
      reason: 'no contributing sentiment items in buffer',
      observed: 0,
      threshold: 1
    }
  }
